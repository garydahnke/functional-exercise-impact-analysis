...
for month_number in month_number_list:
    ...
    plt.scatter(weight_train_chart_data["Date"], weight_train_chart_data["Muscle Soreness"],
                c="green", label="Weight Training", marker="o")
    ...
    if output_type == "file":
        ...
        try:
            filename = f"training-load-vs-recovery-for-{year}-{month_name.lower()}.svg"
            print(f"Creating {filename}")
            plt.savefig(filename)
            filename = f"training-load-vs-recovery-for-{year}-{month_name.lower()}.pdf"
            print(f"Creating {filename}")
            plt.savefig(filename)
        except FileNotFoundError:
