"""
Script: total_volume_for_all_exercises.py
Purpose: This program generates Total Volume report for a selected or all exercises from the "Workouts"  
         sheet for each weight training sesssion. Total Volume = Summation (Reps * Weight) for every set
         per exercise in that weight training session. Data is pulled from data-log.ops spreadsheet - 
         sheet 'Workouts'. At this time, data in the spreadsheet is for one calendar year.         
Author: Gary Dahnke
Date: July 2026
"""
import os   
...
# Consistent scaling — set a default figure size with plt.rcParams["figure.figsize"] = (12,6) so all charts use that size.
    ...
    plt.title(f"Training Volume per Exercise - {current_exercise} - (2 Sets)")
...
# Save charts as *.svg and *.pdf files    
    try:
        filename = f"total-volume-for-{current_exercise.lower().replace(" ","-")}.svg"
        print(f"Creating {filename}")
        plt.savefig(filename)
        filename = f"total-volume-for-{current_exercise.lower().replace(" ","-")}.pdf"
        print(f"Creating {filename}")
        plt.savefig(filename)
    except FileNotFoundError:
