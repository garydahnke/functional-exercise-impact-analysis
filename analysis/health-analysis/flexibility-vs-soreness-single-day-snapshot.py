...
Purpose: This program generates side-by-side bar chart (Flexibility vs Soreness — Single‑Day 
         ...
         spreadsheet is for one calendar year. A date to generate a chart for multiple days is
         possible.
         User Set Variables:
...
start_date = pd.to_datetime("2026-07-05")
...
for date in chart_data["Date"]:
    ...
    flexibility_columns = [f"{j} Flexibility" for j in joints]
    soreness_columns = [f"{j} Soreness" for j in joints]
    ...
    plt.figure(figsize=(12, 6))
    plt.bar(x - width/2, flexibility_values, width, label="Flexibility")
    plt.bar(x + width/2, soreness_values, width, label="Soreness")
    plt.xticks(x, joints)
    plt.ylabel(f"Rating (1-10)")
    max_joint_value = int(max(flexibility_values + soreness_values))
    plt.ylim(0, min(10, max_joint_value + 1))
    plt.yticks(range(0, min(10,max_joint_value + 1)))
    ...
    if output_type == "file":
        ...
        try:
            filename = f"flexibility-vs-soreness-single-day-snapshot-{file_date}.svg"
            print(f"Creating {filename}")
            plt.savefig(filename)
            filename = f"flexibility-vs-soreness-single-day-snapshot-{file_date}.pdf"
            print(f"Creating {filename}")
            plt.savefig(filename)
        except FileNotFoundError:
            ...
            print(f"Files for execise {file_date} have been created.")
    else:
