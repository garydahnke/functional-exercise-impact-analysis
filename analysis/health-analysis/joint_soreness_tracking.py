...
if output_type == "file":
    ...
    try:
        print("-" * 60)
        filename = f"{analytics.health_analysis_charts}joint-soreness-tracking-{file_date}.svg"
        print(f"Creating {filename}")
        plt.savefig(filename)
        filename = f"{analytics.health_analysis_charts}joint-soreness-tracking-{file_date}.pdf"
        print(f"Creating {filename}")
        plt.savefig(filename)
    except FileNotFoundError:
