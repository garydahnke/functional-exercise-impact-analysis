...
plt.xticks(x, chart_dates_xticks)
plt.ylabel(f"Rating (1-10)")
max_joint_value = int(max(joint_flexibility + joint_soreness))
...
if output_type == "file":
    ...
    try:
        filename = f"flexibility-vs-soreness-{joint.lower()}-joint-snapshot-{file_date}.svg"
        print(f"Creating {filename}")
        plt.savefig(filename)
        filename = f"flexibility-vs-soreness-{joint.lower()}-joint-snapshot-{file_date}.pdf"
        print(f"Creating {filename}")
        plt.savefig(filename)
    except FileNotFoundError:
