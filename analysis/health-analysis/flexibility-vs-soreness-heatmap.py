...
if output_type == "file":
   # Save charts as *.svg and *.pdf files.    
    try:
        filename = f"soreness-heatmap-for-{file_date_range}.svg"
        print(f"Creating {filename}")
        plt.savefig(filename)
        filename = f"soreness-heatmap-for-{file_date_range}.pdf"
        print(f"Creating {filename}")
        plt.savefig(filename)
    except FileNotFoundError:
...
plt.figure(figsize=(14, 8))
sns.heatmap(flexibility_matrix, annot=True, cmap="Blues")
plt.title(title)
...
if output_type == "file":
   # Save charts as *.svg and *.pdf files.    
    try:
        filename = f"flexbility-heatmap-for-{file_date_range}.svg"
        print(f"Creating {filename}")
        plt.savefig(filename)
        filename = f"flexbility-heatmap-for-{file_date_range}.pdf"
        print(f"Creating {filename}")
        plt.savefig(filename)
    except FileNotFoundError:
        print("Directory does not exist.")
